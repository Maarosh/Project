...
const detailFilm = filmy.find((odkaz) => odkaz.id === filmId)

const elmTitle = document.querySelector(".card-title").textContent = `${detailFilm.nazev}`
const elmDesc = document.querySelector(".card-text").textContent = `${detailFilm.popis}`

const elmPoster = document.querySelector(".img-fluid").src = `${detailFilm.plakat.url}`
const elmWidth = document.querySelector(".img-fluid").width = `${detailFilm.plakat.sirka}`
const elmHeight = document.querySelector(".img-fluid").height = `${detailFilm.plakat.vyska}`

const datumPremiery = detailFilm.premiera

const datumFormat = dayjs(datumPremiery).format("D.M.YYYY")

const fixedDate = document.getElementById("premiera")


const now = dayjs().format("D.M.YYYY")
const interval = dayjs(datumPremiery).diff(dayjs(), "days")

const vypocetDni = (interval) => {
	if (interval === 0) {
		return `je dnes`
	} else if (interval > 0) {
		return `bude za ${interval}`
	} else {
		return `bylo před ${Math.abs(interval)}`
	}
}

const daysFormat = (interval) => {
	if (interval === 1) {
		return " den"
...
elmForm.addEventListener("submit", (e) => {
	e.preventDefault()
	const elmMessage = document.querySelector("#message-input")
	const message = elmMessage.value
	const elmBox = document.querySelector("#terms-checkbox")
	const box = elmBox.checked
	console.log(box)
	if (Number(message.length) === 0) {
		elmMessage.classList.add("is-invalid")
		elmMessage.focus()
	} else if (box === false) {
		elmBox.classList.add("is-invalid")
		elmBox.focus()
	} else {
		elmForm.innerHTML = `<p class="card-text">${message}</p>`
	}
 })
